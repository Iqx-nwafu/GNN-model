# two_stage_switch_batch_tsnet_T30_T90.py
#
...
#   - Switch duration scenarios:
#       * T = 30 s, sweep fast-closing time: 1..10 s
#       * T = 90 s, sweep fast-closing time: 1..30 s
#   - gamma = 0.8 (fast stage completes 80% closure)
#   - Output time step: 0.1 s (internal dt respects TSNet stability; output resampled to 0.1 s)
#   - Output columns include ALL nodes listed in column A of BOTH the before/after group sheets
#   - For each scenario: traverse all consecutive group switches (typically 29)
#
# Dependencies
...
# -----------------------------
# 6) Resampling to fixed output dt
# -----------------------------
...
def simulate_group_switch_two_stage(
    ...
    # Steady (before) for opening-node demand_coeff
    ...
    tf = float(t_pre + switch_T + t_post)
    ...
    for n in change_nodes:
        ...
        # Close: two-stage (k(t)=k0*(1+PA(t)))  => PA: 0 -> -gamma -> -1
        if qb > 0.0 and qa <= 0.0:
            node.pulse_coeff = two_stage_close_PA(
                t_internal,
                t_start=t_pre,
                T=float(switch_T),
                t_fast=float(fast_time_s),
                gamma=float(gamma),
            )
        ...
        # Open: linear (PA: -1 -> 0)
            ...
            node.pulse_coeff = linear_ramp(
                t_internal, t_start=t_pre, duration=float(switch_T), y0=-1.0, y1=0.0
            )
            node.pulse_status = True

        # Nonzero -> nonzero: linear scaling (PA: 0 -> r-1)
        elif qb > 0.0 and qa > 0.0:
            r = qa / qb
            node.pulse_coeff = linear_ramp(
                t_internal, t_start=t_pre, duration=float(switch_T), y0=0.0, y1=(r - 1.0)
            )
            node.pulse_status = True

    tm = tsnet.simulation.Initializer(tm, 0.0, engine)

    # Opening nodes: set demand_coeff so that q_after = k0*sqrt(p_before)
    eps_p = 1e-3
    for n in change_nodes:
        qb = float(d0.get(n, 0.0))
        qa = float(d1.get(n, 0.0))
...
def simulate_scenario(
    ...
    scenario_name: str,
    switch_T: float,
    fast_times: Iterable[int],
    gamma: float = 0.6,
    output_root: str = "switch_outputs_two_stage_T",
    source_node: str = "J0",
    t_pre: float = 5.0,
    t_post: float = 5.0,
    dt_output: float = 0.1,
    ...
    out_dir = Path(output_root) / scenario_name
    ...
    fast_times_list = list(fast_times)
    total_runs = len(switches) * len(fast_times_list)
    run_idx = 0
    ...
    for sw_idx, (gid0, sh0, gid1, sh1) in enumerate(switches, start=1):
        ...
        for ft in fast_times_list:
            run_idx += 1
            prefix = sw_folder / f"fast_{int(ft):02d}s"

            print(
                f"[{scenario_name}] Run {run_idx}/{total_runs} | "
                f"Switch {sw_idx}/{len(switches)}: G{gid0:02d}->{gid1:02d} | "
                f"T={switch_T}s fast={ft}s gamma={gamma}"
            )
            ...
            try:
                ...
                print(f"[ERROR] [{scenario_name}] G{gid0:02d}->{gid1:02d} T={switch_T}s fast={ft}s failed: {e!r}")
                continue

    summary_df = pd.DataFrame(summary_rows)
    summary_path = out_dir / "switch_fasttime_summary.csv"
    summary_df.to_csv(summary_path, index=False, encoding="utf-8-sig")
    print(f"[{scenario_name}] Summary saved to: {summary_path.resolve()}")

    return summary_df


# -----------------------------
# 9) Main: run T=30 and T=90 scenarios
# -----------------------------
...
if __name__ == "__main__":
    ...
    # Scenario A: T = 30 s, fast time 1..10 s
    simulate_scenario(
        inp_file=INP_FILE,
        groups_xlsx=GROUPS_XLSX,
        scenario_name="T30s",
        switch_T=30.0,
        fast_times=range(1, 11),
        gamma=GAMMA,
        output_root="switch_outputs_two_stage_multiT",
        source_node="J0",
        t_pre=T_PRE,
        t_post=T_POST,
        dt_output=DT_OUT,
    ...
    # Scenario B: T = 90 s, fast time 1..30 s
    simulate_scenario(
        inp_file=INP_FILE,
        groups_xlsx=GROUPS_XLSX,
        scenario_name="T90s",
        switch_T=90.0,
        fast_times=range(1, 31),
        gamma=GAMMA,
        output_root="switch_outputs_two_stage_multiT",
        source_node="J0",
        t_pre=T_PRE,
